...
int Processor(config_t* config, ifstream* fin, shared_ptr<TFile> f, shared_ptr<Digitizer> digitizer) {
...
//	if (config->method_active[CCM_t]) CCM::root_deinit();
//	if (config->method_active[DFT_t]) DFT::root_deinit();
//	if (config->method_active[XSQ_t]) XSQ::root_deinit();
	...
	for (m = 0; m < NUM_METHODS; m++) {
		if (config->method_active[m]) {
			...
			T_data[m].reset((TTree*)f->Get(treename[m]));
			for (int i = 1; i < NUM_METHODS; i++) if ((config->method_done[(m+i)%NUM_METHODS]) || (config->method_active[(m+i)%NUM_METHODS])) T_data[m]->AddFriend(treename[(m+i)%NUM_METHODS]);
			T_data[m]->Write("",TObject::kOverwrite);
			T_data[m].reset();
		}
	...
	for (ch = 0; ch < config->nchan; ch++) {
		td[ch].event.reset();
		for (m = 0; m < NUM_METHODS; m++) if (config->method_active[m]) td[ch].methods[m].reset();
	}
	digitizer.reset();
	f.reset();
	return ret;
