...
XSQ_NEW::XSQ_NEW(int ch, int length, shared_ptr<Digitizer> digitizer) : Method(ch, length, digitizer) {
	...
	TVectorT<double>* pSigma = nullptr;
	int i(0), p(0);
	...
	for (p = 0; p < P; p++) {
		try {
			dStdWave[p] = unique_ptr<double[]>(new double[iStdLength]);
			dStdSigma[p] = unique_ptr<double[]>(new double[iStdLength]);
		} catch (bad_alloc& ba) {iFailed |= alloc_error; return;}
		pWave = (TVectorT<double>*)std_file->Get((p ? "gamma_wave_inv" : "neutron_wave_inv"));
		pSigma = (TVectorT<double>*)std_file->Get((p ? "gamma_sigma" : "neutron_sigma"));
		...
		switch(iStdLength) {
			case 225 : // 500 MSa/s
				for (i = 0; i < iStdLength; i++) { // averages
					...
					dStdSigma[p][i] = ((*pSigma)[2*i] + (*pSigma)[2*i+1])/2.;
			...
			case 899 : // 2 GSa/s
				for (i = 0; i < iStdLength; i++) { // interpolates
					...
					dStdSigma[p][i] = (i%2) ? ((*pSigma)[(i+1)/2] + (*pSigma)[(i-1)/2])/2. : (*pSigma)[i/2];
					dStdPeak[p] = min(dStdPeak[p], dStdWave[p][i]);
				} break;
			case 450 : // 1 GSa/s
				for (i = 0; i < iStdLength; i++) {
					dStdWave[p][i] = (*pWave)[i];
					dStdWave[p][i] = (*pSigma)[i];
					dStdPeak[p] = min(dStdPeak[p], dStdWave[p][i]);
				} break;
...
void XSQ_NEW::SetDefaultParameters(int p) { // for convenience
	XSQ_NEW::sdXsq[p][id]			= -1;
	XSQ_NEW::sdPeakheight[p][id]	= (event->Baseline() - event->Peak_y())*dStdNorm[p]*fGain[p];
...
void XSQ_NEW::SetParameters(void* val, int which, shared_ptr<Digitizer> digitizer) {
	switch (which) {
		case n :
			fGain[n] = *((float*)val);
			for (auto i = 0; i < iStdLength; i++) dStdSigma[n][i] /= fGain[n];
			break;
		case y :
			fGain[y] = *((float*)val);
			for (auto i = 0; i < iStdLength; i++) dStdSigma[y][i] /= fGain[y];
			break;
		default: break;
	}
}
...
double XSQ_NEW::FindChisquare(int p, double dPeak, double dBase, int iOff) {
	double dVal(0), dTrace(0), dDiff(0), dChiSquare(0), dSigma(0);
	int iSample(0);
	for (int i = 0; i < iEventlength; i++) {
		...
		if (iSample < 0) dSigma = dStdSigma[p][0];
		else if (iSample < iStdLength) dSigma = dStdSigma[p][iSample];
		else dSigma = dStdSigma[p][iStdLength-1];
		dDiff = dTrace-dVal;
		dChiSquare += (dDiff*dDiff)/(dSigma*dSigma); // sigma
	}
	return dChiSquare;
}
...
void XSQ_NEW::Analyze() {
	...
	double d000(0), dp00(0), d0p0(0), d00p(0), dpp0(0), dp0p(0), d0pp(0), dm00(0), d0m0(0), d00m(0), dmm0(0), dm0m(0), d0mm(0);
	...
	for (int p = 0; p < P; p++) {
		...
		for (XSQ_NEW::ssIterations[p][id] = 1; XSQ_NEW::ssIterations[p][id] <= iter; XSQ_NEW::ssIterations[p][id]++) {
			...
			dmm0 = FindChisquare(p, dPeak - dEpsilonP,	dBase - dEpsilonB,	iOff);
			dm0m = FindChisquare(p, dPeak - dEpsilonP,	dBase,				iOff - dEpsilonO);
			d0mm = FindChisquare(p, dPeak,				dBase - dEpsilonB,	iOff - dEpsilonO);

			a = (dp00 - 2*d000 + dm00)/(dEpsilonP*dEpsilonP);
			b = (dpp0 - dp00 - d0p0 + 2*d000 - dm00 - d0m0 + dmm0)/(2*dEpsilonB*dEpsilonP);
			c = (dp0p - dp00 - d00p + 2*d000 - dm00 - d00m + dm0m)/(dEpsilonP); // iEpsilonO == 1
			d = (d0p0 - 2*d000 + d0m0)/(dEpsilonB*dEpsilonB);
			e = (d0pp - d0p0 - d00p + 2*d000 - d0m0 - d00m + d0mm)/(dEpsilonB);
			f = (d00p - 2*d000 + d00m);
